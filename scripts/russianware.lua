...
function Library:CreateWindow(...)
    ...
    Library:GiveSignal(InputService.InputBegan:Connect(function(Input, Processed)
        if not Processed and Input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            if key == Enum.KeyCode.End or key == Enum.KeyCode.Insert or key == Enum.KeyCode.Delete then
...
Toggles.esp_enabled:OnChanged(function(val)
    esp.enabled = val
end)

Toggles.esp_box:OnChanged(function(val)
    esp.box[1] = val
end)
Options.esp_boxColor:OnChanged(function(val)
    esp.box[2] = val
end)

Toggles.esp_boxFill:OnChanged(function(val)
    esp.boxfill[1] = val
end)
Options.esp_boxFillColor:OnChanged(function(val)
    esp.boxfill[2] = val
end)

Toggles.esp_health:OnChanged(function(val)
    esp.barlayout.health.enabled = val
end)
Options.esp_barColor:OnChanged(function(val)
    esp.barlayout.health.color2 = val
end)
Options.esp_barColor2:OnChanged(function(val)
    esp.barlayout.health.color1 = val
end)

Toggles.esp_chams:OnChanged(function(val)
    esp.chams[1] = val
end)
Options.esp_chamsColor:OnChanged(function(val)
    esp.chams[2] = val
end)
Options.esp_chamsColor2:OnChanged(function(val)
    esp.chams[3] = val
end)

Toggles.esp_name:OnChanged(function(val)
    esp.textlayout.name.enabled = val
end)
Options.esp_nameColor:OnChanged(function(val)
    esp.textlayout.name.color = val
end)

Toggles.esp_dist:OnChanged(function(val)
    esp.textlayout.distance.enabled = val
end)
Options.esp_distColor:OnChanged(function(val)
    esp.textlayout.distance.color = val
end)

Toggles.esp_healthtext:OnChanged(function(val)
    esp.textlayout.health.enabled = val
end)
Options.esp_healthtextColor:OnChanged(function(val)
    esp.textlayout.health.color = val
end)

Toggles.esp_tracer:OnChanged(function(val)
    esp.tracer[1] = val
end)
Options.esp_tracerColor:OnChanged(function(val)
    esp.tracer[2] = val
end)

Toggles.esp_viewangle:OnChanged(function(val)
    esp.angle[1] = val
end)
Options.esp_viewangleColor:OnChanged(function(val)
    esp.angle[2] = val
end)

Toggles.esp_OSA:OnChanged(function(val)
    esp.arrow[1] = val
end)
Options.esp_OSAColor:OnChanged(function(val)
    esp.arrow[2] = val
end)

Toggles.esp_skeleton:OnChanged(function(val)
    esp.skeleton[1] = val
end)
Options.esp_skeletonColor:OnChanged(function(val)
    esp.skeleton[2] = val
end)

Toggles.esp_outline:OnChanged(function(val)
    esp.outline[1] = val
end)
Options.esp_outlineColor:OnChanged(function(val)
    esp.outline[2] = val
end)

Toggles.esp_maxdistance:OnChanged(function(val)
    esp.limitdistance = val
end)
Options.esp_maxdistanceValue:OnChanged(function(val)
    esp.maxdistance = val
end)

Options.esp_nametextSide:OnChanged(function(val)
    esp.textlayout.name.pos = val
end)
Options.esp_healthtextSide:OnChanged(function(val)
    esp.textlayout.health.pos = val
end)
Options.esp_healthSide:OnChanged(function(val)
    esp.barlayout.health.pos = val
end)
Options.esp_distancetextSide:OnChanged(function(val)
    esp.textlayout.distance.pos = val
end)

Options.esp_arrowRadius:OnChanged(function(val)
    esp.arrowradius = val
end)
Options.esp_arrowSize:OnChanged(function(val)
    esp.arrowsizemodifier = val
end)
Options.esp_arrowTrans:OnChanged(function(val)
    esp.arrowfilltrans = val / 100
end)

Options.esp_textFont:OnChanged(function(val)
    esp.textfont = val
end)
Options.esp_textSize:OnChanged(function(val)
    esp.textsize = val
end)

Toggles.esp_teamcheck:OnChanged(function(val)
    esp.teamcheck = val
end)
...
local RandomGuns = { "USP", "P2000", "Glock", "DualBerettas", "P250", "FiveSeven", "Tec9", "CZ", "DesertEagle", "R8" ,"MP9", "MAC10", "MP7", "UMP", "P90", "Bizon", "M4A4", "M4A1", "AK47", "Famas", "Galil", "AUG", "SG", "AWP", "Scout", "G3SG1","CT Knife" ,"T Knife", "Banana", "Bayonet", "Bearded Axe", "Butterfly Knife", "Cleaver", "Crowbar", "Falchion Knife", "Flip Knife", "Gut Knife", "Huntsman Knife", "Karambit", "Sickle"}  ---J. u fucking loser
 function FireTest(L_1145_arg0, L_1146_arg1, L_1147_arg2, L_1148_arg3, L_1149_arg4) 
	if GetCamera:FindFirstChild("Arms") and GetMe.Character then
		...
		Remote:FireServer(L_1145_arg0, L_1149_arg4 or encodePos(L_1145_arg0.Position + Vector3.new(0,0,0)), RandomGuns[math.random(20,#RandomGuns)], -6846846, nil, nil, 698547860949, Toggles.killAll_arg9.Value, Toggles.killAll_arg10.Value, Vector3.new(), -6985865461, Vector3.new(), true, nil,
        nil,
        nil,
        nil,
        nil,
...
function fireHitpart(L_1145_arg0, L_1146_arg1, L_1147_arg2, L_1148_arg3, L_1149_arg4)
    Remote:FireServer(L_1145_arg0, L_1149_arg4 or encodePos(L_1145_arg0.Position + Vector3.new()), "Karambit", 0, localPlayer.Character.Gun, nil, 9999*999999999*9999, Toggles.killAll_arg9.Value, Toggles.killAll_arg10.Value, Vector3.new(), 0, Vector3.new(1, 0, 0), true, nil,
    nil,
    nil,
    nil,
    nil,
                        ...
                        if plr.Character and plr.Team ~= localPlayer.Team and IsAlive(plr) then
                            ...
                            for i = 0,2 do
                                ...
                                for i = 1,Options.killAll_hps.Value do
                                    ...
                                    oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13,     nil,
                                    nil,
                                    nil,
                                    nil,
                                    nil,
                            ...
                            elseif Options.killAll_type.Value == 'xosmane\'s' then -- самый лучший килл алл не согласен молчать
                                ...
                                local oh13 = true
                                for i = 1,Options.killAll_hps.Value do
                                    game:GetService("ReplicatedStorage").Events.HitPart:FireServer(oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13,nil,
                                    nil,
                                    nil,
                                    nil,
                                    nil,
...
namecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if self == HitPart and method == "fireserver" and Toggles.HuiPart.Value then
        ...
        oh12 = Vector3.new()
        oh13 = true
        return namecall(self, oh1, oh2, oh3, oh4, oh5, oh6, oh7, oh8, oh9, oh10, oh11, oh12, oh13, nil,
        nil,
        nil,
        nil,
        nil,
...
SaveManager:SetFolder('russianware')
